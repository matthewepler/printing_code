/*
Generative Typography Platform - alpha

An interactive interface for generating custom 
generative typography.

This could use a lot of building out. Several basic drawing
tools could be added, along with better controls. 
Feel free to do whatever you want with this.

I know it's messy. But hey, it's free.
git@github.com:matthewepler/generative_type_platform.git
...
void draw() {
  ...
  if ( setFrame == 0 ) {
    ...
    RFont font = new RFont( "FreeSans.ttf", fontSize, RFont.LEFT ); // set base font here
    ...
    for ( int c = 1; c < letterArray.length; c++ ) {
      ...
      if ( letter.contentEquals(" ") ) { // corrects for whitespace characters
        letter = "a";
        noFill();
        noStroke();
      ...
      else if ( prevLetter.contentEquals(" ") ) { // corrects for whitespace characters
        prevLetter = ".";
        grp = font.toGroup( letter ) ;
        pGrp = font.toGroup( prevLetter );
        RPoint[] pnts = grp.getPoints();
