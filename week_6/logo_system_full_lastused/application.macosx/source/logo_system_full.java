...
public class logo_system_full extends PApplet {
...
float boxWidth = 3;
float offset = -0.8f;
float solo_offset = ( offset/1.5f );
int dim = 50;
float dim_cutter = 1.0f;
...
public void setup() {
  ...
  allColors[ 0 ] = color( 0xffFF3999 );   // pink = Soft Lab
  allColors[ 1 ] = color( 0xffFFF41A );   // yellow = Processing / Code Art
  allColors[ 2 ] = color( 0xff55BFD8 );   // teal = Physical Computing
  allColors[ 3 ] = color( 0xff491086 );   // purple = General Department Use
  allColors[ 4 ] = color( 0xff5EF552 );   // green = Camp
  allColors[ 5 ] = color( 0xffFF381A );   // red-orange =  Event
  allColors[ 6 ] = color( 0xff1F1E1E );   // dark grey = Thesis
  allColors[ 7 ] = color( 0xffE3E3E3 );   // light grey = Archive
...
public void drawCorner( int c, float len, boolean _solo ) {
  switch( c ) {
  case 1:  // corner 1 - 2 - 3
...
public void initControls() {
  ...
  controlP5.addSlider("drawCut",0.15f,8).linebreak();
  ...
  controlP5.addSlider("dim_cutter",0.25f,8).linebreak();
  Toggle set = controlP5.addToggle("set");
  set.setColorForeground( 0xffF00505 );
  set.setColorLabel( 0xffFFFFFF ); 
  set.setColorActive( 0xffF00505 );
  set.setColorBackground( 0xff9D2626 );  
  controlP5.end();
...
class Corner {
  ...
  public void display( int c, float _len, boolean _solo ) {
    float len;
    float pos;
    fill( c );
    if( _solo ) {
...
//    }
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "logo_system_full" };
    if (passedArgs != null) {
